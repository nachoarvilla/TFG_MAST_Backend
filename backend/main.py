...
@app.delete("/teams/{team_id}/members", status_code=status.HTTP_200_OK)
def remove_team_member(team_id: int, request: AddMemberRequest, current_user: models.User = Depends(get_current_user), db: Session = Depends(get_db)):
    """Remove a member from a team. Only the team leader can perform this action."""
    # Check if the team exists
    team = db.query(models.Team).filter(models.Team.id == team_id).first()
    if not team:
        raise HTTPException(
...
def delete_team(team_id: int, current_user: models.User = Depends(get_current_user), db: Session = Depends(get_db)):
    """Delete a team. Only the team leader can perform this action."""
    # Check if the team exists
    team = db.query(models.Team).filter(models.Team.id == team_id).first()
    if not team:
        raise HTTPException(
    ...
    # Remove team members first for referential integrity
